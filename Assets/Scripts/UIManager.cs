...
public class UIManager : MonoBehaviourPun
{
	...
	[Header("ListViews")]
	[SerializeField] GameObject _otbListViewPrefab;
	...
	{
		if (_otbListView == null)
		{
			GameObject theListView = Instantiate(_otbListViewPrefab, _otbListSpawnPoint.position, Quaternion.identity);
			theListView.name = theListView.name.Replace("(Clone)", "").Trim();
			theListView.transform.SetParent(_otbListSpawnPoint);
			theListView.transform.localScale = Vector3.one;
			_otbListView = theListView;
			theListView.GetComponent<OTBListViewManager>()._pManager = _pManager;
		}
		//play click sound
		AudioManager.Instance.PlaySound(AudioManager.Instance._buttonClick);
	...
	public void OnOtbListViewValueChanged(int index)
	{
		_otbMessageText.text = "";
		_stopBuying = false;
		StopCoroutine("BuyOptionRoutine");
		...
		if (index >= 0)
		{
			_downPaymentInput.interactable = true;
			_downPaymentInput.Select();
			_selectedCard = _pManager._myOtbs[index];
			_otbCost = _selectedCard.totalCost;
			_minDownPayment = (int)(_otbCost * 0.2f);
			_downPaymentInput.placeholder.GetComponent<Text>().text = _minDownPayment.ToString();
	...
	//	column.Text = title;
	//	_otbListView.Columns.Add(column);
	//}
